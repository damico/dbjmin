...
public class SqlData extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request,
		...
		if (query.contains(";")) {
			ArrayList<ExecuteUpdateObject> sqldata = ActionsFactory.callJetty(
					form_data, Constants.W_BATCH_SQL_ACTION).getResult();
			out.println("<table border = '1' width = '500'>");
			out.println("<tr bgcolor='#CCCCCC'><td><b>#</b></td>"
					+ "<td><b>Status</b></td>" + "<td><b>Statement</b></td>"
					+ "</tr>\n");
			for (int i = 0; i < sqldata.size(); i++) {
				out.println("<tr valign='top'><td>"
						...
						+ "</td>"
						+ "<td>"
						+ sqldata.get(i).getStatus()
						+ "</td>"
						+ "<td>"
						+ OutputDataValidation.getInstance().wrapResult(
								sqldata.get(i).getStatement()) + "</td>"
						+ "</tr>\n");
			}
			out.println("</table>");
			...
			ArrayList data = ActionsFactory.callJetty(form_data,
					Constants.W_SINGLE_SQL_ACTION).getResult();
			...
			if (setSize < 1) {
				...
				for (int j = 0; j < columns_name.size(); j++) {
					out.println("<td><b>" + columns_name.get(j)
							+ "</b>&nbsp</td>");
				}
				out.println("</tr>\n");
				...
				for (int i = 0; i < setSize; i++) {
					...
					for (int j = 0; j < columns_data.size(); j++) {
						try {
							out.println("<td>" + columns_data.get(j)
									+ "&nbsp</td>");
		...
		if (errors.size() > 0) {
			...
			out.println("</table><br><br><table bgcolor = 'black' width='500'>"
					+ "<tr valign='top'><td><font color = 'red'><b>"
					+ sb.toString() + "</b></font></td></tr></table>");
