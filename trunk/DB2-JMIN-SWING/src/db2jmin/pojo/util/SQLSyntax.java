...
/**	 	
 *This class is used for give suggests hints to complete the sqlcommands.
 * The vocabulary contains a word that starts with what has been typed.
 * */
...
public class SQLSyntax implements DocumentListener {
	...
	public SQLSyntax(JTextArea sqltext){
	    ...
	    Scanner sc;
		try {
			sc = new Scanner(new File("sql_syntax"));
			 while (sc.hasNext()){
		    	   sqlDict.add(sc.nextLine());
		       }
        ...
        if (n < 0 && -n <= sqlDict.size()) {
            ...
            if (match.startsWith(prefix)) {
                ...
                SwingUtilities.invokeLater(
                        new SQLCompletion(completion, pos + 1));
            }
        } else {
           mode = Mode.INSERT;
        }
